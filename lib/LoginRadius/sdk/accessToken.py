...
class AccessToken:
    ...
    def validate(self, access_token):
        # Token Validate (GET)
        payload = {'key': self._lr_object._get_api_key(
        ), 'secret': self._lr_object._get_api_secret(), 'access_token': access_token}
    ...
    def activeSessionDetails(self, access_token):
        ...
        payload = {'key': self._lr_object._get_api_key(
        ), 'secret': self._lr_object._get_api_secret(), 'token': access_token}
        url = self._lr_object.SECURE_API_URL + tokenEndpoint + "/activesession"
        return self._lr_object._get_json(url, payload)

    def refresh(self, access_token, expiresIn=''):
        # Refresh Token (GET)
        payload = {'key': self._lr_object._get_api_key(), 'secret': self._lr_object._get_api_secret(
        ), 'access_token': access_token, 'expiresIn': expiresIn}
    ...
    def getTwitterToken(self, tw_access_token, tw_token_secret):
        ...
        payload = {'key': self._lr_object._get_api_key(
        ), 'tw_access_token': tw_access_token, 'tw_token_secret': tw_token_secret}
