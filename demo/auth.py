...
def login():
    ...
    res = \
        loginradius.authentication.login_by_email(email_authentication_model=payload,
...
def passwordless():
    res = \
        loginradius.password_less_login.passwordless_login_by_email(request.args['email'
...
def verify_passwordless():
    res = \
        loginradius.password_less_login.passwordless_login_verification(request.args['token'
            ], '')
    if 'ErrorCode' in res:
        return abort(Response(res['Description'], 400))
    else:
...
def account():
    ...
    res = \
        loginradius.authentication.user_registration_by_email(auth_user_registration_model=payload,
            verification_url=email_verification_url,
            sott=sott_data['Sott'], fields=None)
...
def verify_email():
    res = \
        loginradius.authentication.verify_email(verification_token=request.args['token'
            ])
    if 'ErrorCode' in res:
        return abort(Response(res['Description'], 400))
    else:
...
def forgot_password():
    res = \
        loginradius.authentication.forgot_password(request.form['email'
            ], reset_password_url)
    if 'ErrorCode' in res:
        return abort(Response(res['Description'], 400))
    else:
...
def reset_password():
    payload = {'resettoken': request.form['token'],
               'password': request.form['password']}
    res = \
        loginradius.authentication.reset_password_by_reset_token(payload)
    if 'ErrorCode' in res:
        return abort(Response(res['Description'], 400))
    else:
        return jsonify(res)
...
def mfa():
    res = loginradius.mfa.mfa_login_by_email(email=request.form['email'
            ], password=request.form['password'],
            verification_url=email_verification_url)
    if 'ErrorCode' in res:
        return abort(Response(res['Description'], 400))
...
def verify_mfa():
    res = \
        loginradius.mfa.mfa_validate_google_auth_code(request.form['code'
            ], request.form['token'])
    if 'ErrorCode' in res:
        return abort(Response(res['Description'], 400))
    else:
